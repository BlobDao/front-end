...
export class LPBond extends Bond {
    ...
    public getBlobAmount(networkID: Networks, provider: StaticJsonRpcProvider) {
    ...
    private async getReserves(networkID: Networks, provider: StaticJsonRpcProvider, isToken: boolean): Promise<number> {
        ...
        const isBlob = token1.toLowerCase() === addresses.BLOB_ADDRESS.toLowerCase();

        return isToken ? this.toTokenDecimal(false, isBlob ? reserve0 : reserve1) : this.toTokenDecimal(true, isBlob ? reserve1 : reserve0);
    }

    private toTokenDecimal(isBlob: boolean, reserve: number) {
        return isBlob ? reserve / Math.pow(10, 9) : reserve / Math.pow(10, 18);
    }
}
