...
import { mim, wmatic, } from "../../helpers/bond";
...
export const calcBondDetails = createAsyncThunk("bonding/calcBondDetails", async ({ bond, value, provider, networkID }: ICalcBondDetails, { dispatch }) => {
    ...
    // } else if (bond.name === wmatic.name) {
    //     purchased = purchased / Math.pow(10, 18);
    //     const maticPrice = getTokenPrice("MATIC");
    //     purchased = purchased * maticPrice;
...
export const bondAsset = createAsyncThunk("bonding/bondAsset", async ({ value, address, bond, networkID, provider, slippage, useMatic }: IBondAsset, { dispatch }) => {
